...
static uint16_t FifoTxDataTotalLen = 0;
static uint16_t FifoTxDataCnt = 0;
static uint8_t *FifoTxDataBuf = NULL;
...
//			printf("full\r\n");
			...
			while (buf_idx < ( fifo_len - 1 ) )
			{
//				uart_tx_one_char(UART0, READ_PERI_REG(UART_FIFO(UART0)) & 0xFF);

				UARTRXBuffer[ UARTRXCount ] = READ_PERI_REG( UART_FIFO( UART0 ) );
				if( UARTRXCount != 0 )
				{
					UARTRXCount ++;
				}
				...
				{
					if( UARTRXBuffer[ UARTRXCount ] == 0x55 )
					{
						UARTRXCount = 1;
					}
				}
...
//				uart_tx_one_char(UART0, READ_PERI_REG(UART_FIFO(UART0)) & 0xFF);
			...
			if( UARTRXCount != 0 )
			{
				...
				xQueueSendToBackFromISR( CmdRxHandlTaskQueueHandle , &UARTRXCount , &xHigherPriorityTaskWoken );
...
void  
UartFifoTxStart( UART_Port uart_no , uint8_t * Datas , uint16_t DataLen )
{
	uint8_t TxFifoCnt;
	uint8_t i;

	FifoTxDataTotalLen = DataLen;
	FifoTxDataCnt = 0;
	FifoTxDataBuf = Datas;
	
	TxFifoCnt = (READ_PERI_REG(UART_STATUS(uart_no))>>UART_TXFIFO_CNT_S)&UART_TXFIFO_CNT;

	TxFifoCnt = 128 - TxFifoCnt;
	printf("TxFifoCnt:%d!\r\n" , TxFifoCnt );
	for( i = 0; i < TxFifoCnt ; i++ )
	{
		WRITE_PERI_REG(UART_FIFO(uart_no) , FifoTxDataBuf[i] );
	}
	
	FifoTxDataCnt = TxFifoCnt;
	SET_PERI_REG_MASK(UART_INT_ENA(UART0), UART_TXFIFO_EMPTY_INT_ENA);
}

void  
uart0TxFifoEmptyhandl( void )
{
	uint8_t TxFifoCnt;
	uint8_t i;
	
	if( FifoTxDataCnt < FifoTxDataTotalLen )
	{
		TxFifoCnt = (READ_PERI_REG(UART_STATUS( UART0 ))>>UART_TXFIFO_CNT_S)&UART_TXFIFO_CNT;

		TxFifoCnt = 128 - TxFifoCnt;

//		printf("TxFifoCnt:%d!\r\n" , TxFifoCnt );
		if( ( FifoTxDataTotalLen - FifoTxDataCnt ) < TxFifoCnt )
		{
			TxFifoCnt = FifoTxDataTotalLen - FifoTxDataCnt;
		}
			
		for( i = 0; i < TxFifoCnt ; i++ )
		{
			WRITE_PERI_REG(UART_FIFO( UART0 ) , FifoTxDataBuf[ FifoTxDataCnt + i ] );
		}
	...
	else if( UartTxIdleSemaphoreHandle != NULL )
	{
		xSemaphoreGiveFromISR( UartTxIdleSemaphoreHandle, NULL );
	}
}


void  
uart_init_new( uint8_t * UartRxBufferPtr )
{

    UARTRXBuffer = UartRxBufferPtr;
